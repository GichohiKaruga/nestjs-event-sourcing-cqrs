 ...
 * (i.e. product) events are being subscribed to. Upon events being received,
...
export class EventStore implements IEventPublisher, IMessageSource {
  ...
  async publish<T extends IEvent>(event: T) {
    ...
    const productId = message.productId || message.productDto.productId;
    const streamName = `${this.category}-${productId}`;
